...
class SelfplayProcess(mp.Process):
    ...
    def execute_episode_with_example_storing(self):
        ...
        printl(f'{self.name}: Episode {self.episode_count} completed in {round(time.time() - start_time, 2)}s with {turn_count} actions and generating {len(training_examples)} examples. Result was {result}')
